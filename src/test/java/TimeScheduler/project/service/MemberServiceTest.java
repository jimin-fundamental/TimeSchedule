...
class MemberServiceTest {
    ...
    MemoryMemberRepository memberRepository; //여기서 new를 통해 repository 객체 생성하면, MemberService 클래스에서 이미 저장소 만들었는데 그거와 별개의 저장소를 만들게 됨
    ...
    public void 회원가입() {//원래 join인데 어차피 테스트코드이므로 의미통하게 한국어로 바꾸는 것도 좋아
        ...
        assertEquals(member.getEmail(),findMember.getEmail()); //repository의 이름과 member로 받은 이름이 같은지 검증

    }

    @Test
    public void 중복_회원_예외() throws Exception{ //Test에서는 예외상황도 제대로 작동되는지 체크해주는 게 중요
        //Given
